...
void* decode_func(void* arg){
    ...
    for(;;) {
        ...
        if(stream_index == player->video_stream_index) {//视频流
            ...
            LOGE("decode video stream = %d", video_frame_count++);
        } else if(stream_index == player->audio_stream_index) {//音频流
            ret = decode_audio(player, packet);
            LOGE("decode audio stream = %d", audio_frame_count++);
        }
...
JNIEXPORT void JNICALL Java_com_shunyi_ffmpegmodule_MainActivity_release
        (JNIEnv * env, jclass clazz){
    //释放内存以及关闭文件
